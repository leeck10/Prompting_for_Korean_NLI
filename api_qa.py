...
def score_for_input(args, premise, hypothesis, cands=[], knowledge=None):
    ...
    response = openai.ChatCompletion.create(
                    model='gpt-3.5-turbo',
                    messages=[{"role": "system", "content": f"다음의 예시와 같이 의문형으로 변환\n 가설: 인교진 씨는 무대 뒤편을 바라보았다.\n변환: 인교진 씨는 무대 뒤편을 바라보았습니까?"},
    ...
    # Chat 기반 모델 
        ...
        prefix = '다음 문맥을 보고 질문에 답하세요 예/아니오/알 수 없음'
        if args.task =='baseline':
            if args.shot == 0:
                prompts = [{"role": "system", "content": f'{prefix} 문맥: {premise}'},
                            {"role": "user", "content": f'질문: {hypothesis} 답:'}]
            elif args.shot > 0:
                prompts = [{"role": "system", "content": f'{prefix} 예시\n{example}\n 문맥: {premise}'},
                            {"role": "user", "content": f'질문: {hypothesis} 답:'}]
...
def main():
    ...
    if args.task == 'baseline':
        ...
        args.input_path = '/home/cyh/GKP-master/data/kor_nli/knowledge/knowledge_gpt3.dev5.json'
    args.output_path = f'data/kor_nli/inference/qa.{args.shot}shot.{args.task}.{args.model_type}.json'
